        ...
        .then(data => {
            currentGeoJsonLayer = L.geoJSON(data, {
                onEachFeature: function (feature, layer) {
                    ...
                    const parcelId = feature.properties.parcelpin; // Was Parcel_ID
                    const acres = feature.properties.acres;       // Was Acres
                    const todScore = feature.properties.TOD_Index; // Was TOD_Score

                    // Add a tooltip
                    layer.bindTooltip(`Parcel ID: ${parcelId}<br>Acres: ${acres}<br>TOD Score: ${todScore}`);
                }
            }).addTo(map);
        })
        .catch(error => {
...
function toggleParcelSelection(feature, layer) {
    ...
    const parcelAcreage = parseFloat(feature.properties.acres); // Correct property name and convert to number

    // Check if the parcel is already selected
    const parcelIndex = selectedParcels.findIndex(p => p.properties.parcelpin === feature.properties.parcelpin); // Correct property name
